...
function Platform() {
  ...
  const save = useMutation({ mutationFn:async(value:Form)=>{ if(value.id){ const {error}=await supabase.rpc("platform_update_tenant",{p_tenant:value.id,p_name:value.name,p_subdomain:value.subdomain,p_tier:value.tier as "basic",p_status:value.status,p_contact_email:value.contact_email,p_contact_phone:value.contact_phone}); if(error) throw error; } else { const {error}=await supabase.rpc("platform_create_tenant",{p_name:value.name,p_subdomain:value.subdomain,p_tier:value.tier as "basic",p_contact_email:value.contact_email,p_contact_phone:value.contact_phone}); if(error) throw error; } }, onSuccess:()=>{toast.success(form?.id?"Church updated":"Church shell created");setForm(null);qc.invalidateQueries({queryKey:["platform-overview"]});},onError:(error)=>toast.error(error instanceof Error?error.message:"Could not save church") });
          ...
          {pendingChurches.map((c: any) => (
            <div key={c.id} className="flex flex-wrap items-center justify-between gap-4 p-4">
              <div>
                ...
                <Badge variant="secondary" className="mt-1 capitalize">{planLabel(c.tier)} package</Badge>
              </div>
